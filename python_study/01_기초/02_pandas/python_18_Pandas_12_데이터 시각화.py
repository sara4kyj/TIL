# -*- coding: utf-8 -*-
"""
Created on Wed Dec 29 14:13:30 2021
...
plt.ylabel('yname', rotation=0, loc='top', labelpad=30, fontdict=font1)
...
plt.hist(s1,
         bins=5,
         density=False) # True로 설정 시, 막대 아래 총 면적이 1이 되는 밀도함수 출력
                        # 즉, y축 값이 확률로 변경되어 출력됨
plt.hist(s1, density=False) # 확률 값으로 출력
s1.plot(kind='kde')     # 커널 밀도 함수(kernal density dstimation) 출력
                        # (연속형 히스토그램)
...
plt.boxplot(iris_x)
plt.xticks(ticks=[1,2,3,4], label=x_names)
